...
EVAL_MODEL = os.environ.get("EVAL_MODEL", "Qwen/Qwen3-Next-80B-A3B-Instruct")
...
def need_citation(question, answer, sentence):
    ...
    for t in range(5):
        ...
        output = query_llm(msg, model=EVAL_MODEL, temperature=0 if t == 0 else 1.0, max_new_tokens=10, stop="Analysis:", return_usage=True)
        ...
        if score is None:
            print("Unexpected need_citation output: ", output)
            if output and 'Trigger' in output:
                break
            continue
        else:
            break
...
def is_support(question, statement, context):
    ...
    for t in range(5):
        ...
        if score is None:
            print("Unexpected support output: ", output)
            if output and 'Trigger' in output:
                break
            continue
        else:
...
def score_recall(question, answer, statements_with_citations):
    ...
    for js in statements_with_citations:
        ...
        if len(matched_citations) > 0:
            ...
            if score is None:
                raise NotImplementedError(f"ERROR Unexpected support output: {statement} \t>>\t {output}")
            else:
                scores.append(score)
        else:
            ...
            if score is None:
                raise NotImplementedError(f"ERROR Unexpected need_citation output: {output}")
            else:
...
def is_relevant(question, statement, citation):
    ...
    for t in range(5):
        ...
        if score is None:
            print("Unexpected relevant output: ", output)
            if output and 'Trigger' in output:
                break
            continue
        else:
...
def score_precision(question, answer, statements_with_citations):
    ...
    for js in statements_with_citations:
        ...
        for c in citations:
            ...
            if score is None:
                raise NotImplementedError(f"ERROR Unexpected relevant output: {output}")
            else:
                scores.append(score)
    if not scores:
        return 0, statements_with_citations, usages
...
def get_citation_score(js, max_statement_num=None):
    ...
    if max_statement_num and len(statements_with_citations) > max_statement_num:
        print(f"Too many statements, only evaluate {max_statement_num} of {len(statements_with_citations)} statements.")
    ...
    js['gpt_usage'] = {
        'prompt_tokens': sum(x.get('prompt_tokens', 0) for x in (usages1 + usages2)),
        'completion_tokens': sum(x.get('completion_tokens', 0) for x in (usages1 + usages2)),
    }
    return js
